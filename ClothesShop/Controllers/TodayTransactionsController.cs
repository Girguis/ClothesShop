...
{
    [Authentication]
    public class TodayTransactionsController : BaseController
    {
        ...
        public ActionResult Index()
        {
            ...
            var result = transactions.Where(p => p.CreatedOn.Value.ToString(dateFormat) == DateTime.UtcNow.ToString(dateFormat))
                .Select(p => p);
            ...
            var eResult = expenses.Where(p => p.CreatedOn.Value.ToString(dateFormat) == DateTime.UtcNow.ToString(dateFormat))
                .Select(p => p);
        ...
        public ActionResult GetWeekSummary(TransactionsSearchViewModel model)
        {
            ...
            {
                var transactions = _TodayTransactionsRepo.GetAll();
                var result = transactions.Where(p => p.CreatedOn >= model.StartDate.AddHours(-1 * GetUtcOffset()) && p.CreatedOn < model.EndDate.AddHours(-1 * GetUtcOffset()).AddDays(1))
                    .Select(p => p);

                var totalSales = result.Sum(p => p.Transactions.Sum(t => t.NumberOfPieces * t.SellingPrice)).Value;

                var expenses = _TodayExpensesRepo.GetAll();
                var eResult = expenses.Where(p => p.CreatedOn >= model.StartDate.AddHours(-1 * GetUtcOffset()) && p.CreatedOn < model.EndDate.AddHours(-1 * GetUtcOffset()).AddDays(1))
                    .Select(p => p);
        ...
        public ActionResult GetAll(SearchViewModel obj, TransactionsSearchViewModel model)
        {
            try
            {
                ...
                var result = products.Where(p => p.Transactions.Any(c =>
                {
                    var date = c.TodayTransaction.CreatedOn.Value.AddHours(utcOffset);
                    return date >= model.StartDate && date < model.EndDate.AddDays(1);
                })).Select(p => new TransactionsViewModel()
                {
                    ProductName = p.Name,
                    DayOne = p.Transactions.Where(t => t.TodayTransaction.CreatedOn.Value.AddHours(utcOffset).ToString(dateFormat) == model.StartDate.AddDays(0).ToString(dateFormat)).Sum(pp => pp.NumberOfPieces).Value,
                    DayTwo = p.Transactions.Where(t => t.TodayTransaction.CreatedOn.Value.AddHours(utcOffset).ToString(dateFormat) == model.StartDate.AddDays(1).ToString(dateFormat)).Sum(pp => pp.NumberOfPieces).Value,
                    DayThree = p.Transactions.Where(t => t.TodayTransaction.CreatedOn.Value.AddHours(utcOffset).ToString(dateFormat) == model.StartDate.AddDays(2).ToString(dateFormat)).Sum(pp => pp.NumberOfPieces).Value,
                    DayFour = p.Transactions.Where(t => t.TodayTransaction.CreatedOn.Value.AddHours(utcOffset).ToString(dateFormat) == model.StartDate.AddDays(3).ToString(dateFormat)).Sum(pp => pp.NumberOfPieces).Value,
                    DayFive = p.Transactions.Where(t => t.TodayTransaction.CreatedOn.Value.AddHours(utcOffset).ToString(dateFormat) == model.StartDate.AddDays(4).ToString(dateFormat)).Sum(pp => pp.NumberOfPieces).Value,
                    DaySix = p.Transactions.Where(t => t.TodayTransaction.CreatedOn.Value.AddHours(utcOffset).ToString(dateFormat) == model.StartDate.AddDays(5).ToString(dateFormat)).Sum(pp => pp.NumberOfPieces).Value,
                    DaySeven = p.Transactions.Where(t => t.TodayTransaction.CreatedOn.Value.AddHours(utcOffset).ToString(dateFormat) == model.StartDate.AddDays(6).ToString(dateFormat)).Sum(pp => pp.NumberOfPieces).Value,
                });
